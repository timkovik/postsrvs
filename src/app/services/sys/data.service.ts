...
export class DataService {
  ...
  public getInitialData() {
    this.storage.ready().then(() => {
      this.storage
        .keys()
        .then((keys) => console.log("записи в стораже: ", keys));
      this.storage
        .get("orders")
  ...
  public async getApiData() {
    if (this.settings.rules.appMode.includes("auto")) {
      ...
      this.getBalance(Number(this.auth.userId), 1).subscribe(
        (res: Response) => {
          this.sys
            .getOrders(res.res_more.map((order: Order) => order.id.toString()))
  ...
  public saveOrders(orders: Order[]) {
    ...
    this.ordersMap.forEach((order, key) => {
      ...
      if (order.status_id > incomOrdersMap.get(key).status_id) {
        ...
        console.log(
          `sys:: Заказ ${key} не будет обновлен (входящий status_id меньше текущего)`
        );
      }
    });
  ...
  public ordersCountWithStatus(status: string) {
    ...
    return this.orders.pipe(
      map(
        (orders) =>
          orders.filter((order) => statusIds[status].includes(order.status_id))
            .length
      )
    );
  }
